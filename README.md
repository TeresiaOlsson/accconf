...
AccConf is a Python package to configure and build Python devices for particle accelerators.
...
- A light-weight device database for particle accelerators which use the EPICS control system.
  The implementation will however be sufficiently generic to also support other control systems which has need for a way to store configuration data for devices.

- Built-in support for configuring both pyAML devices to be used with pyAML applications or ophyd/ophyd-async devices to be used with Bluesky applications,
